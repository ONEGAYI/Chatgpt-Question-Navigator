...
- `PopupApp.tsx` — 主组件：存储用量仪表、对话列表、缓存操作按钮（逐条删除、清空全部、LRU 清理）、滚屏速率设置（default/fast/turbo 三档切换）
...
- `{ type: 'SET_SCROLL_PROFILE', name }` — 切换滚屏速率档位
...
| `MessageScanner` | 核心扫描引擎。通过 MutationObserver（防抖 500ms，流式输出期间提升至 3000ms）和 IntersectionObserver 监控 DOM 变化，将候选消息交给 CacheStore 去重合并。候选生成时通过 DomAdapter.findTurnKeyForElement 提取 turnKey。`isDirectMountCandidateTurn` 过滤只有 viewport 邻近（±1 viewport buffer）的节点才进入 mountedIds，防止虚拟化残留 DOM 节点被误判。rescan 时注册 AI turn 锚点元素到 elementById（扩展 visibleRange），computeActiveMessageId 同时追踪 user 和 assistant 消息 |
...
| `JumpController` | 跳转控制：已挂载消息通过 `tryLandOnMounted` 后置验证确认目标进入 viewport 后 scrollIntoView + 高亮；未挂载消息渐进式跳转（scrollRatio 粗定位 + orderKey 自适应步进，JumpToken 可取消，MAX_ATTEMPTS=200）；用户滚动/Esc/新跳转自动取消当前操作 |
...
- **防抖/节流常量**：定义在各自模块顶部（`MUTATION_DEBOUNCE_MS=500`, `STREAMING_DEBOUNCE_MS=3000`（AI 流式输出期间 rescan 防抖）, `SCROLL_THROTTLE_MS=300`, `SAVE_DEBOUNCE_MS=2000`）
